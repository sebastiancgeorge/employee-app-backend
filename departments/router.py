...
async def create_department(body: DepartmentCreate, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    department = await service.create(db, body.name, body.employee_ids)
    return department

@router.get("", response_model=list[DepartmentResponse])
async def get_all_departments(db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def get_department_by_name(department_name: str, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def get_department_by_id(department_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    result = await service.get_department_by_id(department_id, db)
    return result

@router.put("/{department_id}", response_model=DepartmentResponse)
async def update_department(department_id: int, body: DepartmentUpdate, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    result = await service.update_department(department_id, body.name, body.employee_ids, db)
    return result

@router.delete("/{department_id}", status_code=status.HTTP_204_NO_CONTENT)
async def soft_delete_department(department_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
