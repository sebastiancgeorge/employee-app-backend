...
async def get_department_by_id(department_id: int, db: AsyncSession):
    stmt = select(Department).options(selectinload(Department.employees)).where(Department.id == department_id, Department.deleted_at.is_(None))
    result = await db.scalars(stmt)
    department = result.first()
    return department

async def get_department_by_name(department_name: str, db: AsyncSession):
    stmt = select(Department).options(selectinload(Department.employees)).where(Department.name == department_name, Department.deleted_at.is_(None))
    result = await db.scalars(stmt)
    department = result.first()
    return department
