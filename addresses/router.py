...
async def create_address(employee_id: int, body: AddressCreate, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def get_addresses_by_employee(employee_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def get_address_by_id(address_id: int,db: AsyncSession = Depends(get_db),  ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    result = await service.get_address_by_id(address_id, db)
    return result

@router.put("/address/{address_id}", response_model=AddressResponse)
async def update_address(address_id: int,body: AddressCreate, db: AsyncSession = Depends(get_db),  ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def soft_delete_address(address_id: int,db: AsyncSession = Depends(get_db),):#_current_user: TokenPayload = Depends(get):#_current_user)):
