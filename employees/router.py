...
async def create_employee(body: EmployeeCreate, db: AsyncSession = Depends(get_db)):
    employee = await service.create(
        db, name=body.name, email=body.email, password=body.password, age=body.age, address=body.address, role=body.role
    )
    return employee
...
async def get_employee_by_name(
    employee_name: str, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)
):
...
async def get_employee_by_id(
    employee_id: int, db: AsyncSession = Depends(get_db), _current_user: TokenPayload = Depends(get_current_user)
):
...
@router.delete(
    "/{employee_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role(EmployeeRole.HR))]
)
