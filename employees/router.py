...
async def create_employee(body : EmployeeCreate, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    employee = await service.create(db, body.name, body.email, body.password, body.age, body.address)
    return employee

@router.get("",response_model= list[EmployeeResponse])
async def get_all_employees(db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    results = await service.get_all_employees(db)
    return [r for r in results.all()]

@router.get("/search/{employee_name}")
async def get_employee_by_name(employee_name: str, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    result= await service.get_employee_by_name(employee_name, db)
    return result


@router.get("/{employee_id}", response_model = EmployeeResponseId)
async def get_employee_by_id(employee_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    result= await service.get_employee_by_id(employee_id, db)
    return result

@router.put("/{employee_id}")
async def update_employee(employee_id: int, body: EmployeeCreate, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
    name = body.name
    email = body.email
    age = body.age
...
async def soft_delete_employee(employee_id: int,db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def attach_department(employee_id: int, department_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def detach_department(employee_id: int, department_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
...
async def delete_employee_address(employee_id: int, address_id: int, db: AsyncSession = Depends(get_db), ):#_current_user: TokenPayload = Depends(get):#_current_user)):
